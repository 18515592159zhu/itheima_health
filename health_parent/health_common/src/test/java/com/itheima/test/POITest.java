...
public class POITest {
    ...
    public void test1() throws Exception {
        ...
        XSSFWorkbook excel = new XSSFWorkbook(new FileInputStream(new File("c:\\poi.xlsx")));
        //读取Excel文件中第一个Sheet标签页,获取工作表，既可以根据工作表的顺序获取，也可以根据工作表的名称获取
        XSSFSheet sheet = excel.getSheetAt(0);
        //遍历Sheet标签页，获得每一行数据
        for (Row row : sheet) {
            //遍历行，获得每个单元格对象,获得单元格中的值
            for (Cell cell : row) {
    ...
    public void test3() throws Exception {
        ...
        FileOutputStream out = new FileOutputStream(new File("c:\\hello.xlsx"));
        excel.write(out);
        out.flush();
        excel.close();
    }
